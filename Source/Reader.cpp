...
namespace Ini
{
    int Reader::ParseEquals(std::string_view line, std::array<std::string_view, 3>& parts, const bool allowMaps)
    {
        ...
        if (const auto idx1 = line.find('=', idx0 + 1); idx1 != std::string::npos && !allowMaps)
        {
            ...
            for (size_t i = idx0; i < line.length(); i++)
            {
                    ...
                    if (std::ranges::all_of(part, [](const char c) { return isdigit(c) || c == '-'; }))
                    {
                        ...
                        if (part.find('.'))
                        {
                            ...
                            if (errno == ERANGE)
                            {
                                if (throwOnIniWarning)
                                {
                                    throw IniParsingException(
                                        fileName, "Numerical value had a decimal point while not being a valid floating point value.", currentLine);
                                }
    ...
    void Reader::ThrowOnIniWarning(const bool doThrow) { this->throwOnIniWarning = doThrow; }
} // namespace Ini
